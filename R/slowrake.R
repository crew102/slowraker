...
#' Slow RAKE
#'
#' A relatively slow version of the Rapid Automatic Keyword Extraction (RAKE) algorithm. See \href{Automatic keyword extraction from individual documents}{http://media.wiley.com/product_data/excerpt/22/04707498/0470749822.pdf} for details on how RAKE works.
#'
#' @param txt A character vector, where each element of the vector contains the text for one document.
#' @param stop_words A vector of stop words which will be removed from your documents. The default value (\code{smart_words}) contains the "SMART" stop words (equivalent to \code{\link[tm]{stopwords("SMART")}}). A value of \code{NULL} indicates that no stop words will be removed.
#' @param word_min_char The minimum number of characters that a word must have to remain in the corpus. Words with fewer than \code{word_min_char} characters will be removed before the RAKE algorithm is applied. Also note that filtering words based on \code{word_min_char} happens before stemming, so you should not consider the length of a word's stem when choosing \code{word_min_char}.
#' @param stem Do you want to stem the words in your documents before extracting keywords?
#' @param keep_pos A vector of part-of-speech (POS) tags that specifies acceptable parts-of-speech for keywords to have. The default value \code{c("NN", "NNS", "NNP", "NNPS")} specifies that all words that have a POS that is noun-related (e.g., plural noun, proper noun, etc.) will be considered when creating the list of candidate keywords. To see the other possible POS tags along with their descriptions, see the \code{\link{pos_tags}} data frame (\code{View(slowraker::pos_tags)}), or see their descriptions in \href{Part-Of-Speech Tagging with R}{http://martinschweinberger.de/docs/articles/PosTagR.pdf}. Specifying \code{keep_pos = NULL} will not filter out any words based on their POS.
#'
...
#'
#'    \item{score}{The keyword's score, as per the RAKE algorithm. Keywords with higher scores are considered to be more higher quality, as compared to those with lower scores.}
#'
#'    \item{stem}{If you specified \code{stem = TRUE}, you will also get the stemmed versions of the keywords in this column. When you choose stemming, the keyword's score (\code{score}) will be based off of its stem, but the reported number of times that the keyword appears (i.e., \code{freq}) will still be based off of the raw, unstemmed version of the keyword.}
#'  }
#'
#' @examples
