...
In this example, `slowrake()` took `r sr_time` seconds to execute while `rapidrake()` took `r rr_time` seconds. In other words, `rapidrake()` was about `r round(sr_time / rr_time, 0)` times faster than `slowrake()`.
